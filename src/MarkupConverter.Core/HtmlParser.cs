...
namespace MarkupConverter.Core
{
    ...
    internal sealed class HtmlParser
    {
        ...
        private readonly HtmlLexicalAnalyzer htmlLexicalAnalyzer;

        // Document from which all elements are created
        private readonly XmlDocument document = new XmlDocument();

        // Stack for open elements
        private readonly Stack<XmlElement> openedElements = new Stack<XmlElement>();
        private readonly Stack<XmlElement> pendingInlineElements = new Stack<XmlElement>();
        ...
        private HtmlParser(string inputString)
        {
            ...
            htmlLexicalAnalyzer = new HtmlLexicalAnalyzer(inputString);

            // Get first token from input, expecting text
            htmlLexicalAnalyzer.GetNextContentToken();
        }
        ...
        internal static string ExtractHtmlFromClipboardData(string htmlDataString)
        {
            ...
            // which could be wrong assumption. We need to implement more flexible parsing here
        ...
        /// Returns XmlElement representing the top-level HTML element.
        /// </summary>
        private XmlElement ParseHtmlContent()
        {
            ...
            var htmlRootElement = document.CreateElement("html", XhtmlNamespace);
            OpenStructuringElement(htmlRootElement);

            while (htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.EOF)
            {
                if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.OpeningTagStart)
                {
                    htmlLexicalAnalyzer.GetNextTagToken();
                    if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
                    {
                        string htmlElementName = htmlLexicalAnalyzer.NextToken.ToLowerInvariant();
                        htmlLexicalAnalyzer.GetNextTagToken();

                        // Create an element
                        var htmlElement = document.CreateElement(htmlElementName, XhtmlNamespace);

                        // Parse element attributes
                        ParseAttributes(htmlElement);

                        if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.EmptyTagEnd || HtmlSchema.IsEmptyElement(htmlElementName))
                        {
                            // It is an element without content (because of explicit slash or based on implicit knowledge about HTML)
                        ...
                        else if (HtmlSchema.IsInlineElement(htmlElementName))
                        {
                            ...
                            // overlapping tags into normal hierarchical element structure.
                        ...
                        else
                        {
                            ...
                            // Thus we will ignore them on closing tag as well.
                            // Anyway we don't know what to do with them on conversion to XAML.
                        }
                    }
                    else
                    {
                        ...
                        //  Add the following assertion here, right? or output "<" as a text run instead?:
                        // InvariantAssert(false, "Angle bracket without a following name is not expected");
                    }
                }
                else if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.ClosingTagStart)
                {
                    htmlLexicalAnalyzer.GetNextTagToken();
                    if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
                    {
                ...
                else if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Text)
                {
                    AddTextContent(htmlLexicalAnalyzer.NextToken);
                }
                else if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Comment)
                {
                    AddComment(htmlLexicalAnalyzer.NextToken);
                }
            ...
            if (htmlRootElement.FirstChild is XmlElement firstChild &&
                firstChild == htmlRootElement.LastChild &&
                string.Equals(firstChild.LocalName, "html", StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        {
            var htmlElementCopy = document.CreateElement(htmlElement.LocalName, XhtmlNamespace);
        ...
        {
            InvariantAssert(openedElements.Count > 0, "AddEmptyElement: Stack of opened elements cannot be empty, as we have at least one artificial root element");
            var htmlParent = openedElements.Peek();
            htmlParent.AppendChild(htmlEmptyElement);
        }

        private void OpenInlineElement(XmlElement htmlInlineElement)
        {
            pendingInlineElements.Push(htmlInlineElement);
        }

        // Opens structuring element such as Div or Table etc.
        private void OpenStructuringElement(XmlElement htmlElement)
        {
            // Close all pending inline elements
            // All block elements are considered as delimiters for inline elements which forces all inline
            // elements to be closed and re-opened in the following structural element (if any).
            ...
            {
                while (openedElements.Count > 0 && HtmlSchema.IsInlineElement(openedElements.Peek().LocalName))
                {
                    var htmlInlineElement = openedElements.Pop();
                    InvariantAssert(openedElements.Count > 0, "OpenStructuringElement: stack of opened elements cannot become empty here");

                    pendingInlineElements.Push(CreateElementCopy(htmlInlineElement));
                }
            }
            ...
            if (openedElements.Count > 0)
            {
                ...
                {
                    openedElements.Pop();
                    htmlParent = openedElements.Count > 0 ? openedElements.Peek() : null;
                }
            ...
            openedElements.Push(htmlElement);
        }

        private bool IsElementOpened(string htmlElementName)
        {
            return openedElements.Any(e => e.LocalName == htmlElementName);
        }

        private void CloseElement(string htmlElementName)
        {
            // Check if the element is opened and already added to the parent
            InvariantAssert(openedElements.Count > 0, "CloseElement: Stack of opened elements cannot be empty, as we have at least one artificial root element");

            // Check if the element is opened and still waiting to be added to the parent
            if (pendingInlineElements.Count > 0 && pendingInlineElements.Peek().LocalName == htmlElementName)
            {
                ...
                var htmlInlineElement = pendingInlineElements.Pop();
                InvariantAssert(openedElements.Count > 0, "CloseElement: Stack of opened elements cannot be empty, as we have at least one artificial root element");
                var htmlParent = openedElements.Peek();
            ...
            {
                while (openedElements.Count > 1) // we never pop the last element - the artificial root
                {
                    // Close all unbalanced elements.
                    var htmlOpenedElement = openedElements.Pop();
                    ...
                    if (HtmlSchema.IsInlineElement(htmlOpenedElement.LocalName))
                    {
                        ...
                        pendingInlineElements.Push(CreateElementCopy(htmlOpenedElement));
                    }
                }
            }
        ...
        private void AddTextContent(string textContent)
        {
            ...
            InvariantAssert(openedElements.Count > 0, "AddTextContent: Stack of opened elements cannot be empty, as we have at least one artificial root element");
        ...
        private void AddComment(string comment)
        {
            ...
            InvariantAssert(openedElements.Count > 0, "AddComment: Stack of opened elements cannot be empty, as we have at least one artificial root element");

            var htmlParent = openedElements.Peek();
            var xmlComment = document.CreateComment(comment);
            htmlParent.AppendChild(xmlComment);
        }

        // Moves all inline elements pending for opening to actual document and adds them to current open stack.
        private void OpenPendingInlineElements()
        {
            if (pendingInlineElements.Count > 0)
            {
                var htmlInlineElement = pendingInlineElements.Pop();

                OpenPendingInlineElements();

                InvariantAssert(openedElements.Count > 0, "OpenPendingInlineElements: Stack of opened elements cannot be empty, as we have at least one artificial root element");

                var htmlParent = openedElements.Peek();
                htmlParent.AppendChild(htmlInlineElement);
                openedElements.Push(htmlInlineElement);
            }
        }
        ...
        {
            while (htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.EOF &&
                   htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.TagEnd &&
                   htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.EmptyTagEnd)
            {
                // read next attribute (name=value)
                if (htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
                {
                    string attributeName = htmlLexicalAnalyzer.NextToken;
                    htmlLexicalAnalyzer.GetNextEqualSignToken();

                    htmlLexicalAnalyzer.GetNextAtomToken();

                    string attributeValue = htmlLexicalAnalyzer.NextToken;
